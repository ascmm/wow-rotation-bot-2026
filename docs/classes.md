# AEMIS — Supported WoW Classes & Specs (All 13 Classes, 80 Hero Specs)

AEMIS supports every playable class and specialization in World of Warcraft Midnight, including all 80 hero talent trees.

## Death Knight

| Spec | Hero Trees |
|---|---|
| Blood | Deathbringer · San'layn |
| Frost | Rider of the Apocalypse · Deathbringer |
| Unholy | Rider of the Apocalypse · San'layn |

## Demon Hunter

| Spec | Hero Trees |
|---|---|
| Havoc | Aldrachi Reaver · Fel-Scarred |
| Vengeance | Aldrachi Reaver · Fel-Scarred |

## Druid

| Spec | Hero Trees |
|---|---|
| Balance | Keeper of the Grove · Elune's Chosen |
| Feral | Druid of the Claw · Wildstalker |
| Guardian | Druid of the Claw · Elune's Chosen |
| Restoration | Keeper of the Grove · Wildstalker |

## Evoker

| Spec | Hero Trees |
|---|---|
| Devastation | Flameshaper · Scalecommander |
| Preservation | Flameshaper · Chronowarden |
| Augmentation | Scalecommander · Chronowarden |

## Hunter

| Spec | Hero Trees |
|---|---|
| Beast Mastery | Pack Leader · Sentinel |
| Marksmanship | Dark Ranger · Sentinel |
| Survival | Pack Leader · Dark Ranger |

## Mage

| Spec | Hero Trees |
|---|---|
| Arcane | Spellslinger · Sunfury |
| Fire | Frostfire · Sunfury |
| Frost | Frostfire · Spellslinger |

## Monk

| Spec | Hero Trees |
|---|---|
| Brewmaster | Master of Harmony · Conduit of the Celestials |
| Mistweaver | Master of Harmony · Conduit of the Celestials |
| Windwalker | Shado-Pan · Conduit of the Celestials |

## Paladin

| Spec | Hero Trees |
|---|---|
| Holy | Herald of the Sun · Lightsmith |
| Protection | Herald of the Sun · Lightsmith |
| Retribution | Templar · Herald of the Sun |

## Priest

| Spec | Hero Trees |
|---|---|
| Discipline | Archon · Oracle |
| Holy | Archon · Oracle |
| Shadow | Archon · Voidweaver |

## Rogue

| Spec | Hero Trees |
|---|---|
| Assassination | Deathstalker · Fatebound |
| Outlaw | Deathstalker · Trickster |
| Subtlety | Deathstalker · Fatebound |

## Shaman

| Spec | Hero Trees |
|---|---|
| Elemental | Farseer · Stormbringer |
| Enhancement | Totemic · Stormbringer |
| Restoration | Farseer · Totemic |

## Warlock

| Spec | Hero Trees |
|---|---|
| Affliction | Hellcaller · Soul Harvester |
| Demonology | Diabolist · Soul Harvester |
| Destruction | Hellcaller · Diabolist |

## Warrior

| Spec | Hero Trees |
|---|---|
| Arms | Colossus · Slayer |
| Fury | Colossus · Slayer |
| Protection | Colossus · Mountain Thane |

---

Each hero tree has a dedicated rotation file. AEMIS never merges two hero trees into one generic fallback — every combination gets its own optimized APL.

**→ [Get Access at aemis.io](https://aemis.io) · [Join Discord](https://discord.gg/aemis)**
